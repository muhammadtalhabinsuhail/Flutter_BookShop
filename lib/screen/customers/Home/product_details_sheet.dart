...
import 'package:project/screen/model/ProductModel.dart';
import 'package:shared_preferences/shared_preferences.dart';
import '../../TextSystem/rich_text_formatter.dart';
import '../Wishlist/wishlist_controller.dart';
import 'add_to_cart_button.dart';

class EnhancedProductDetailsSheet extends StatefulWidget {
  final ProductModel product;
  final VoidCallback? onCartUpdated;

  const EnhancedProductDetailsSheet({
    Key? key,
    required this.product,
    this.onCartUpdated,
  }) : super(key: key);
  ...
  @override
  State<EnhancedProductDetailsSheet> createState() => _EnhancedProductDetailsSheetState();
}

class _EnhancedProductDetailsSheetState extends State<EnhancedProductDetailsSheet>
    with TickerProviderStateMixin {
  final WishlistController _wishlistController = WishlistController();
  ...
  Widget _buildProductImage() {
    try {
      if (widget.product.imgurl.startsWith('data:image') ||
          widget.product.imgurl.length > 100) {
        String base64String = widget.product.imgurl;
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // Enhanced Description with Rich Text Formatting
                  Container(
                    width: double.infinity,
                    ...
                    child: RichText(
                      text: TextSpan(
                        children: RichTextFormatter.parseFormattedText(
                            widget.product.description ?? 'No description available'
                        ),
                      ),
                    ),
